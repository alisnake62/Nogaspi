...
def test_messaging_initiateConversation():
    ...
    assert funcRtr['newConversationId'] == sqlSelect(table='conversation')[0]['id']
...
def test_messaging_initiateConversation_if_conversation_already_exist_with_other_user():
    ...
    assert funcRtr['isInitiate']
    assert funcRtr['newConversationId'] == sqlSelect(table='conversation', conditions="WHERE id > 2")[0]['id']
