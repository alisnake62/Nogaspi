...
def test_user_postProfilePicture():
    ...
    assert os.path.isfile(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.jpg")
    os.remove(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.jpg")
...
def test_user_postProfilePicture_if_file_already_exist():
    generateAndSaveFile(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/", "toto.png")
    ...
    assert os.path.isfile(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.png")

    funcRtr = postProfilePicture(FakeRequest({"token": "token_toto"}, files={"profilePicture": 'totoFile.jpg'}))

    assert not os.path.isfile(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.png")
    assert os.path.isfile(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.jpg")
    os.remove(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.jpg")
    assert funcRtr['isPosted']
    assert sqlSelect(table='userNogaspi', conditions="WHERE id = 1")[0]['profilePicture'] == "toto.jpg"

    if os.path.isfile(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.png"):
        os.remove(f"{os.environ['DIRECTORY_PROJECT']}assets/images/users/toto.png")
