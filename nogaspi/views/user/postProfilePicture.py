...
def postProfilePicture(request):
    ...
    with EngineSQLAlchemy() as session:
        ...
        if user.profilePicture is not None and os.path.isfile(f"{os.environ['DIRECTORY_ASSET']}images/users/{user.profilePicture}"):
            os.remove(f"{os.environ['DIRECTORY_ASSET']}images/users/{user.profilePicture}")

        try:
            extension = secure_filename(profilePicture.filename).split('.')[-1]
            filename = f"{user.pseudo}.{extension}"
            profilePicture.save(f"{os.environ['DIRECTORY_ASSET']}images/users/{filename}")
        except Exception:
